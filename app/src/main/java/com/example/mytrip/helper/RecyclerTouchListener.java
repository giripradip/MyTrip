...
import com.example.mytrip.custominterface.OnActivityTouchListener;
...
//        mSwipeListener = listener;
    ...
    public void openSwipeOptions(int position) {
        ...
        RecyclerView.ViewHolder holder = rView.findViewHolderForAdapterPosition(position);

        if (!swipeable || holder == null || holder.itemView == null
...
//                        }
    ...
    private boolean shouldIgnoreAction(int touchedPosition) {
        return rView == null || ignoredViewTypes.contains(Objects.requireNonNull(rView.getAdapter()).getItemViewType(touchedPosition));
    }
